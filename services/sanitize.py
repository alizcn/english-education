...
def clean_job_title(raw: str) -> str:
    s = (raw or '').strip()
    if not s:
        raise SanitizationError('Pozisyon adı boş olamaz.')
    if '\n' in s or '\r' in s or '\t' in s:
        raise SanitizationError('Pozisyon adı tek satırda olmalı.')
    ...
    if _BANNED_PATTERNS.search(s):
        raise SanitizationError('Pozisyon adı geçersiz ifadeler içeriyor.')
    return s
